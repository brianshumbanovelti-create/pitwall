...
// Strategy screen: starting tyre + drive mode per driver. No pit plan, no team orders.
...
function startWeekend(){
  ...
  WEEKEND = {
    ...
    strategy: {},
    isWetWeekend: STATE.wetWeekends && STATE.wetWeekends.includes(STATE.round),
    driverForm: rollWeekendForm(),
  };
  myTeam().drivers.forEach(d=>{
    WEEKEND.strategy[d.abbr] = { tyre: 'M', mode: 'balanced' };
  });
  showScreen('screen-weekend');
  renderWeekend();
}

// Roll a form modifier per driver for this weekend — persists through quali and race
function rollWeekendForm(){
  const form = {};
  TEAMS.forEach(team=>{
    const scale = tierFormScale(team.tier);
    team.drivers.forEach(drv=>{
      // Skill reduces the swing — a top driver has fewer off weekends
      const raw = randf(-2.5, 2.5) * scale * (1 - drv.skill/180);
      form[drv.abbr] = clamp(raw, -3.5, 3.5);
    });
  });
  return form;
}
...
function renderWeekend(){
  ...
  document.getElementById('wkMeta').innerHTML = `${track.laps} laps · ${track.tempC}°C air / ${track.trackTempC}°C track · 💨 ${track.windKmh} km/h · ${track.character}${WEEKEND.isWetWeekend ? ' · 🌧 WET WEEKEND' : ''}`;
...
function applyQualiChaos(baseTime, driver, teamTier){
  ...
  let variance = 0;
  switch(chaos){
    case 'normal':   variance = randf(-3.0, 3.0) * tierMul * (1 - driver.skill/200); break;
    case 'shuffled': variance = randf(-3.8, 3.8) * tierMul * (1 - driver.skill/180); break;
    case 'wet':      variance = randf(-4.5, 4.5) * tierMul * (1 - driver.skill/150); break;
    case 'chaotic':  variance = randf(-5.5, 5.5) * tierMul * (1 - driver.skill/140); break;
  }
  return baseTime - variance;
}

// Random narrative explanation when a driver's result deviates from expectation
function pickQualiNarrative(gained, tierMul){
  const over = [
    'hit a perfect lap, tow from ahead',
    'nailed the final sector',
    'first out on a clean track',
    'timed the improving track perfectly',
    'used the slipstream well',
    'soft tyre peak perfectly timed',
    'others lost time — clean run',
  ];
  const under = [
    'messed up final flying lap',
    'clipped the wall, compromised exit',
    'hit traffic on the hot lap',
    'rain arrived mid-lap, sector 3 lost',
    'went out too early on an improving track',
    'tyre graining on the out-lap',
    'yellow flags ruined the push lap',
    'lost rear on entry, lap aborted',
  ];
  const badly = [
    'crashed at the final corner, session over',
    'spin in the final sector',
    'mechanical issue in quali',
    'stuck in traffic on all three runs',
  ];
  if(gained >= 5) return pick(badly);
  if(gained >= 1.5) return pick(over);
  if(gained <= -5) return pick(badly);
  if(gained <= -1.5) return pick(under);
  return null;
}
...
function runQualiStage(stage){
  ...
  if(stage==='q1'){
    const times = allDrivers.map(d=>{
      ...
      const form = WEEKEND.driverForm[d.abbr] || 0;
      return { ...d, time: withChaos + form };
    ...
    out.forEach(d=>{
      const expected = expectedQualiSlot(d);
      const actual = times.indexOf(d) + 1;
      const gained = expected - actual;
      const note = pickQualiNarrative(gained, 1);
      lines.push(`<div class="bad">OUT: P${actual} ${d.name} ${d.time.toFixed(2)}${note?` — <span class="note">${note}</span>`:''}</div>`);
    });
    advancing.slice(0,5).forEach((d,i)=>{
      const expected = expectedQualiSlot(d);
      const actual = i + 1;
      const gained = expected - actual;
      const note = pickQualiNarrative(gained, 1);
      lines.push(`<div>P${actual} ${d.name} ${d.time.toFixed(2)}${note?` — <span class="note">${note}</span>`:''}</div>`);
    });
  ...
  if(stage==='q2'){
    ...
    out.forEach(d=>{
      const expected = expectedQualiSlot(d);
      const actual = pool.indexOf(d) + 11;
      const gained = expected - actual;
      const note = pickQualiNarrative(gained, 1);
      lines.push(`<div class="bad">OUT: P${actual} ${d.name} ${d.time.toFixed(2)}${note?` — <span class="note">${note}</span>`:''}</div>`);
    });
  ...
  if(stage==='q3'){
    ...
    pool.forEach((d,i)=>{
      ...
      const gained = expected - actual;
      const note = pickQualiNarrative(gained, 1);
      lines.push(`<div>${actual}. ${d.name} ${d.time.toFixed(2)}${note?` — <span class="note">${note}</span>`:''}</div>`);
    });
...
// Expected qualifying slot for narrative purposes — used to spot "off weekend" results
function expectedQualiSlot(driver){
  // Rank all 22 drivers by pace + skill
  const ranked = TEAMS.flatMap(t=>t.drivers.map(d=>({
    abbr: d.abbr,
    score: (t.pace + (t.id===STATE.myTeamId?STATE.carPaceBoost:0)) * 0.6 + d.skill * 0.4,
  }))).sort((a,b)=>b.score-a.score);
  return ranked.findIndex(r=>r.abbr === driver.abbr) + 1;
}
...
function buildStrategyBlock(){
  ...
  el.innerHTML = `
    ...
    <div class="wk-step-body">
      <p class="dim small" style="margin-bottom:10px">Choose each driver's starting tyre and engine mode.</p>
...
function driverStratHtml(d){
  ...
  const form = WEEKEND.driverForm[d.abbr] || 0;
  let formBadge = '';
  if(form > 1.5) formBadge = '<span style="color:var(--green);font-size:10.5px">▲ great form</span>';
  else if(form < -1.5) formBadge = '<span style="color:var(--red);font-size:10.5px">▼ off weekend</span>';
  return `
    <div class="driver-strat" data-abbr="${d.abbr}">
      <div class="driver-strat-name">
        <span>${d.name}</span>
        <span style="color:var(--dim);font-size:11px;display:flex;gap:8px;align-items:center">
          ${formBadge}
          <span>Grid P${gridPos}</span>
        </span>
      </div>
...
function bindStrategyControls(scope){
  ...
  root.querySelectorAll('[data-mode]').forEach(b=>b.addEventListener('click', ()=>{
    WEEKEND.strategy[b.dataset.abbr].mode = b.dataset.mode;
    refreshStrategyBlock();
  }));
}
...
function buildGoRacingBlock(){
  ...
  if(btn && ready) btn.addEventListener('click', ()=>{
    ...
    // Apply the weekend form to STATE.form so it affects the race start grid
    STATE.form = { ...(STATE.form || {}), ...WEEKEND.driverForm };
