...
function rollQualiChaos(track){
  const r = Math.random();
  // Wet races are always more chaotic
...
// Apply the chaos factor to a driver's quali time — lower variance = predictable top teams
function applyChaos(baseTime, driver, team){
  const chaos = WEEKEND.qualiChaos;
  let variance = 0;
  // Tier-based base variance
  const tierMul = { title: 0.8, contender: 1.0, midfield: 1.2, backmarker: 1.4 }[team.tier] || 1.0;
  // Higher chaos = more wild swings. Driver skill reduces randomness.
  switch(chaos){
  ...
  return baseTime - variance; // subtract because lower time = faster
}
...
function runQualiStage(stage){
  ...
  if(stage==='q1'){
    const times = allDrivers.map(d=>{
      ...
      const withChaos = applyChaos(base, d, { tier: d.teamTier });
