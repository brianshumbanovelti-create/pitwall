...
function loadFromSlot(slot){
  try{
    ...
    if(!s.polesByAbbr) s.polesByAbbr = {};
...
// Static driver lookup (name/skill etc.) independent of any live RACE state —
// searches every team's original roster since STATE.driverTeam tracks current team.
function driverInfoByAbbr(abbr){
...
function openDriverCareerModal(abbr){
  const info = driverInfoByAbbr(abbr);
  const teamId = STATE.driverTeam[abbr];
  const t = teamId ? teamById(teamId) : null;
  const stats = getDriverCareerStats(abbr);
  const points = STATE.driversPoints[abbr] || 0;
  openModal(`
    <div class="driver-detail">
      <div class="dd-header">
        <div>
          <div class="dd-name">${info ? info.name : abbr}</div>
          <div class="dd-team">${t ? t.name : ''} · ${abbr}</div>
        </div>
      </div>
      <div class="dd-grid">
        <div class="dd-cell"><div class="l">Points (season)</div><div class="v">${points}</div></div>
        <div class="dd-cell"><div class="l">Starts</div><div class="v">${stats.starts}</div></div>
        <div class="dd-cell"><div class="l">Poles</div><div class="v">${stats.poles}/${stats.starts}</div></div>
        <div class="dd-cell"><div class="l">Podiums</div><div class="v">${stats.podiums}/${stats.starts}</div></div>
        <div class="dd-cell"><div class="l">Wins</div><div class="v">${stats.wins}/${stats.starts}</div></div>
      </div>
...
// Career totals derived from STATE.raceLog (never cleared across seasons) plus
// the durable polesByAbbr counter. Wins/podiums/starts come straight out of
// raceLog so there's a single source of truth and no risk of drift.
function getDriverCareerStats(abbr){
  let starts=0, wins=0, podiums=0;
  STATE.raceLog.forEach(race=>{
    const r = race.results.find(x=>x.abbr===abbr);
    if(!r) return;
    starts++;
    if(!r.retired){
      if(r.pos===1) wins++;
  ...
  const poles = (STATE.polesByAbbr && STATE.polesByAbbr[abbr]) || 0;
  return { starts, wins, podiums, poles };
}
...
function renderHubMain(){
  ...
  main.innerHTML = `
    ...
    <div class="hub-section">
      <div class="hub-section-title">Drivers' Championship</div>
      <div class="hub-section-sub">Tap a driver for career stats.</div>
      <table class="standings-table">
        <thead><tr><th>Pos</th><th>Driver</th><th>Points</th></tr></thead>
        <tbody id="drivTableBody"></tbody>
      </table>
    </div>
  ...
  const drivBody = document.getElementById('drivTableBody');
  getDriversStandings().forEach((s,i)=>{
    const teamId = STATE.driverTeam[s.abbr];
    const t = teamId ? teamById(teamId) : null;
    const info = driverInfoByAbbr(s.abbr);
    const tr = document.createElement('tr');
    tr.style.cursor = 'pointer';
    if(teamId===STATE.myTeamId) tr.className='me';
    tr.innerHTML = `<td class="pos">${i+1}</td><td>${t?`<span class="team-pill" style="background:${t.color}"></span>`:''}${s.abbr} <span class="dim" style="font-size:11px">${info?info.name:''}</span></td><td class="pts">${s.points}</td>`;
    tr.addEventListener('click', ()=>openDriverCareerModal(s.abbr));
    drivBody.appendChild(tr);
  });
...
// Resolves which compound should be SHOWN right now, accounting for a pit
// stop in progress (already fitted the new tyre, old one still on d.tyre
// until the stop timer completes) or a queued-but-not-started request.
function displayTyre(d){
  if(d._pitTimer > 0 && d._pendingTyre) return d._pendingTyre;
  if(d._pitRequested && d._requestedTyre) return d._requestedTyre;
  return d.tyre;
}
  ...
  RACE.drivers.forEach(d=>{
    ...
    row.innerHTML = `
      ...
      <span style="display:flex;align-items:center;gap:4px">
        <span class="tt-tyre tyre-${displayTyre(d)}">${displayTyre(d)}</span>
        ${tyreArcSvg(life)}
      </span>
...
function drawTrackView(){
  ...
  // ---- Serpentine track path: 5 stacked lanes, boustrophedon ----
  // Lane 1 runs left->right, lane 2 right->left, lane 3 left->right, etc.
  // Lanes are joined by short vertical risers at the alternating edge.
  // pts[] stays a flat ordered array — everything downstream (sectors,
  // pointAt interpolation, S/F marker, car dots) only cares that it's an
  // ordered closed loop of {x,y}, so none of that needs to change.
  const padX = w * 0.08;
  const padY = h * 0.12;
  const laneCount = 5;
  const laneLeft = padX;
  const laneRight = w - padX;
  const laneTop = padY;
  const laneBottom = h - padY;
  const laneSpacing = (laneBottom - laneTop) / (laneCount - 1);
  const laneWidth = laneRight - laneLeft;
  const kink = RACE.track.kink || 0.05;

  // Distribute N points across (laneCount horizontal runs) + (laneCount-1 risers),
  // weighted by each segment's length so point density stays roughly even.
  const N = 180;
  const riserLen = laneSpacing;
  const totalLen = laneWidth*laneCount + riserLen*(laneCount-1);
  const pts = [];
  for(let seg=0; seg<laneCount; seg++){
    const y = laneTop + seg*laneSpacing;
    const leftToRight = (seg % 2 === 0);
    const nLane = Math.round(N * (laneWidth/totalLen));
    for(let i=0;i<nLane;i++){
      const f = i/nLane;
      const x = leftToRight ? (laneLeft + f*laneWidth) : (laneRight - f*laneWidth);
      // Gentle wave along each lane so it doesn't look like a ruler-straight line
      const wob = Math.sin(f*Math.PI*4 + seg) * kink * laneSpacing * 0.5;
      pts.push({ x, y: y + wob });
    }
    if(seg < laneCount-1){
      const xAtEdge = leftToRight ? laneRight : laneLeft;
      const nRiser = Math.max(2, Math.round(N * (riserLen/totalLen)));
      for(let i=0;i<nRiser;i++){
        const f = i/nRiser;
        pts.push({ x: xAtEdge, y: y + f*laneSpacing });
      }
    }
  }
  const NP = pts.length;

  const sectorRanges = [[0, Math.floor(NP/3)], [Math.floor(NP/3), Math.floor(2*NP/3)], [Math.floor(2*NP/3), NP]];
  ...
  active.forEach(d=>{
    ...
    ctx.strokeStyle = { S:'#ef4444', M:'#fbbf24', H:'#e5e7eb', I:'#22c55e', W:'#3b82f6' }[displayTyre(d)];
    ctx.lineWidth = 1.6;
    ctx.stroke();
    ctx.fillStyle = '#EDEAE2';
    ctx.font = '10px JetBrains Mono, monospace';
