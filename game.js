...
function showSprintStrategyModal(){
  ...
  const strategy = {};
  myDrivers.forEach(d=>{ strategy[d.abbr] = { tyre: 'M' }; });

  openModal(`
    <h2>Sprint — ${track.name}</h2>
    <p style="font-size:12px;margin-bottom:12px">
      <b>Reverse grid</b> · <b>${sprintLapCount(track)} laps</b> · <b>1 mandatory stop</b> (must use different compound)
    </p>
    ${myDrivers.map(d=>{
      const gridPos = SPRINT.reversedGrid.findIndex(g=>g.abbr===d.abbr) + 1;
      return `
      <div class="driver-strat" style="background:var(--bg2);margin-bottom:12px;padding:12px;border-radius:3px">
        <div class="driver-strat-name">
          <span>${d.name}</span>
          <span class="dim" style="font-size:11px">Starting P${gridPos}</span>
        </div>
        <div style="font-size:11px;color:var(--dim);margin-bottom:4px">Starting tyre</div>
        <div class="tyre-select">
          ${['S','M','H'].map(c=>`
            <button class="tyre-btn sprint-tyre ${strategy[d.abbr].tyre===c?'active':''}" data-abbr="${d.abbr}" data-tyre="${c}">
  ...
  document.querySelectorAll('.sprint-tyre').forEach(b=>b.addEventListener('click', ()=>{
    strategy[b.dataset.abbr].tyre = b.dataset.tyre;
    document.querySelectorAll(`.sprint-tyre[data-abbr="${b.dataset.abbr}"]`).forEach(x=>x.classList.remove('active'));
...
function launchSprintRace(track, grid, strategy){
  ...
  updateSpeedButtons();
  simIntervalMs = (5*60*1000)/sprintLapCount(track);
...
function renderHub(){
  ...
  document.getElementById('sideCap').textContent = `$${STATE.capUsed}M / $${costCapForRaces(TRACKS.length)}M`;
...
function runAduoReviewIfNeeded(){
  ...
  if(mine){
    myTeamMessage = `You qualified for ADUO (+$${mine.allowance.toFixed(2)}M, +${mine.upgrades} PU upgrade)`;
    detail = `Gap to benchmark: ${mine.gapPct.toFixed(2)}%. Allowance added to your budget.`;
  }

  STATE.aduoHistory.push({
    round: roundNum,
    results,
    myTeamMessage,
    detail,
...
function openUpgradesModal(){
  ...
  const render = ()=> openModal(`
    ...
    <p>Budget: <b style="color:var(--amber)">$${STATE.budget}M</b> · Cost cap used <b>$${STATE.capUsed}M / $${cap}M</b></p>
...
function startRace(gridOverride, strategyOverride){
  const track = currentTrack();
  RACE = buildRaceState(track, gridOverride, { isSprint: false });
  RACE.drivers.forEach(d=>{
    d.usedCompounds = new Set();
...
function startRenderLoop(){
  ...
  const loop = ()=>{
    ...
    lastAnimFrame = now;
    if(!racePaused){
      const speedFactor = raceSpeed === 2 ? 0.5 : 0.25;
      trackAnimTime = (trackAnimTime + dt * speedFactor) % 1;
    }
...
function simulateLap(silent){
  ...
  // Sort by total time to determine leader and overtake relationships
  const sorted = [...active].sort((a,b)=>a.totalTime-b.totalTime);

  // Battery drain/recharge + Overtake Mode detection
  active.forEach((d, idx)=>{
    ...
    // Overtake Mode: within 1.0s of car ahead
    d._overtakeActive = (carAhead && gapAhead < 1.0);

    // Battery management
    const mode = ENGINE_MODES[d.engineMode] || ENGINE_MODES.balanced;
    let drain = 2.0 + mode.batteryDrain;   // base + mode extra
    // Boost effect drains separately when active
    if(d.boostEffect && RACE.lap <= d.boostEffect.untilLap) drain += 4.0;
    // Overtake Mode drains slightly
  ...
  if(RACE.scLapsRemaining > 0){
    active.forEach(d=>{
      if(d.teamId === STATE.myTeamId) return;   // player decides for themselves
      if(d._pitTimer > 0) return;
      if(d.retired) return;
      // Any AI within 6 laps of their planned pit window dives in
...
function computeLapTime(d, track, wet){
  ...
  // Boost effect
  let boostBonus = 0;
  if(d.boostEffect && RACE.lap <= d.boostEffect.untilLap){
    boostBonus = -0.6;
  }

  // Overtake Mode
  let overtakeBonus = 0;
  if(d._overtakeActive && track.hasDRS){
    overtakeBonus = -0.55;
  }

  // Engine mode limit penalty
  ...
  let t = base + ageLoss + compoundOffset + tempPenalty + wetPenalty + fuelPenalty + modeBonus + radioBonus + dmgPenalty + freshBonus + boostBonus + overtakeBonus + modeLimitPenalty;
...
function openDriverDetail(d){
  ...
  openModal(`
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:14px">
      <div style="background:var(--panel2);padding:8px 10px;border-radius:3px"><div style="font-size:10px;color:var(--dimmer);text-transform:uppercase">Position</div><div style="font-size:15px;font-weight:700">${d.retired?'DNF':'P'+d.position}</div></div>
      <div style="background:var(--panel2);padding:8px 10px;border-radius:3px"><div style="font-size:10px;color:var(--dimmer);text-transform:uppercase">Interval</div><div style="font-size:15px;font-weight:700">${d.position===1?'—':'+'+d.interval.toFixed(1)+'s'}</div></div>
      <div style="background:var(--panel2);padding:8px 10px;border-radius:3px"><div style="font-size:10px;color:var(--dimmer);text-transform:uppercase">Tyre</div><div style="font-size:15px;font-weight:700">${comp.name} · ${d.tyreAge}L</div></div>
      <div style="background:var(--panel2);padding:8px 10px;border-radius:3px"><div style="font-size:10px;color:var(--dimmer);text-transform:uppercase">Life</div><div style="font-size:15px;font-weight:700">${life}%</div></div>
      <div style="background:var(--panel2);padding:8px 10px;border-radius:3px"><div style="font-size:10px;color:var(--dimmer);text-transform:uppercase">Fuel</div><div style="font-size:15px;font-weight:700">${Math.round(d.fuel)}%</div></div>
      <div style="background:var(--panel2);padding:8px 10px;border-radius:3px"><div style="font-size:10px;color:var(--dimmer);text-transform:uppercase">Battery</div><div style="font-size:15px;font-weight:700">${Math.round(d.battery)}%</div></div>
      <div style="background:var(--panel2);padding:8px 10px;border-radius:3px"><div style="font-size:10px;color:var(--dimmer);text-transform:uppercase">Health</div><div style="font-size:15px;font-weight:700">${health}%</div></div>
    </div>
...
function renderStrip(){
  ...
  strip.querySelectorAll('.radio-btn').forEach(b=>{
    b.addEventListener('click', ()=>{
      ...
      strip.querySelectorAll('.radio-panel').forEach(p=>{ if(p !== panel) p.classList.remove('active'); });
      strip.querySelectorAll('.radio-btn').forEach(x=>x.classList.remove('active'));
      const wasActive = panel.classList.contains('active');
      if(wasActive){ panel.classList.remove('active'); }
      else { panel.classList.add('active'); b.classList.add('active'); }
    });
  });
...
function openPitMenu(driver){
  ...
  const mandatoryNote = RACE.isSprint && !driver.hasPitted
    ? '<div style="background:rgba(232,169,58,0.12);border:1px solid var(--amber);padding:8px 12px;border-radius:3px;font-size:12px;color:var(--amber);margin-bottom:12px">⚠ <b>1 mandatory stop required</b> — you must use a different compound</div>'
    : '';
  openModal(`
...
function sendRadioFor(abbr, key){
  ...
  if(cmd.effect.boost){
    ...
    driver.boostEffect = { untilLap: RACE.lap + (cmd.effect.durationLaps || 2) };
    pushMsg('cmd', 'CMD:', `Boost — deploying full power.`);
    setTimeout(()=>pushMsg('driver', abbr+':', 'Deploying full power!'), 500);
    beep('radio');
    renderStrip();
    return;
  }
...
function drawTrackView(){
  ...
  drawOrder.forEach(d=>{
    // Cars behind the leader have run the SAME number of laps but taken MORE time to do so,
    // so distance covered must be measured against the leader's clock, not raw totalTime.
...
function showRaceResults(){
  ...
  // Sprint DSQ check: driver didn't pit
  if(RACE.isSprint){
    RACE.drivers.forEach(d=>{
      if(!d.hasPitted && !d.retired){
        d.dsqSprint = true;
  ...
  // Sprint mode: no season, no board, no points. Just results.
  if(RACE.isSprint){
    renderSprintResults(results, classification);
    return;
  }

  // Career mode: normal processing
  let myPrize = 0;
  const myResults = [];
  results.forEach((d,i)=>{
