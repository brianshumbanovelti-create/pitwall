...
function loadFromSlot(slot){
  try{
    ...
    if(s.version !== SAVE_VERSION) return null;   // fresh start only
    return s;
  }catch(e){ return null; }
}
...
function openDriverProfile(abbr){
  ...
  const formDots = last5.map(r=>{
    const cls = r >= 7.5 ? 'rating-high' : r >= 5 ? 'rating-mid' : 'rating-low';
    return `<span class="dp-form-dot dot ${cls}" style="background:${
      r >= 7.5 ? 'rgba(95,184,120,0.18)' : r >= 5 ? 'rgba(232,169,58,0.18)' : 'rgba(196,69,61,0.18)'
    };color:${
      r >= 7.5 ? 'var(--green)' : r >= 5 ? 'var(--amber)' : 'var(--red)'
    }">${r.toFixed(1)}</span>`;
  }).join('');
    ...
    <div class="driver-profile">
      ...
      ${last5.length ? `
        <div>
          ...
          <div class="dp-form">${formDots}</div>
        </div>
      ` : ''}
...
function computeDriverRating(entry){
  // entry: { finishPos, startPos, retired, fastestLap, hadContact }
...
function renderStrip(){
  ...
  // Radio headset — toggles inline panel
...
function showRaceResults(){
  ...
  // Build classification with ratings
  ...
  // Also add pole to driver stats (based on start position 1)
  RACE.drivers.forEach(d=>{
