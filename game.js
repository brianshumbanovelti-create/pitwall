...
const PRINCIPAL_STYLES = {
  mer: { pitBias: 1.10 }, fer: { pitBias: 0.88 }, mcl: { pitBias: 1.05 },
  rbr: { pitBias: 1.00 }, vcb: { pitBias: 1.00 }, alp: { pitBias: 0.92 },
  haa: { pitBias: 1.08 }, aud: { pitBias: 1.08 }, wil: { pitBias: 1.00 },
  amr: { pitBias: 0.90 }, cad: { pitBias: 1.10 },
};
...
// Visual animation clock for the tracker
let trackAnimTime = 0;   // in laps (0..1 wrapping)
let lastAnimFrame = 0;
...
function startRenderLoop(){
  ...
  const loop = ()=>{
    ...
    lastAnimFrame = now;
    if(!racePaused){
      // Advance visual clock. At raceSpeed 1, one lap takes 4 seconds.
      // 2× button makes it 2 laps per 4 seconds.
      const lapsPerSecond = 0.25 * raceSpeed * (raceSpeed === 2 ? 2 : 1);
      // Simpler: at 1× → 0.25 laps/s (4s per lap). At 2× → 0.5 laps/s (2s per lap).
...
function drawTrackView(){
  ...
  // Active cars (not retired, not in pit)
  const active = RACE.drivers.filter(d => !d.retired && d._pitTimer === 0);
  if(active.length === 0) return;

  // Sort by total time to identify leader
  const sorted = [...active].sort((a,b)=>a.totalTime - b.totalTime);
  const leader = sorted[0];
  const leaderTime = leader.totalTime;

  // Visual reference: each car's displayed phase
  // - `trackAnimTime` is a continuously advancing 0..1 value shared by all cars
  // - car's own visual phase = (its own race-time laps) + trackAnimTime
  // - everyone moves forward together
  // - the leader doesn't sit pinned at 0; they move like everyone else
  ...
  drawOrder.forEach(d=>{
    // Each driver's visual phase within the lap
    // d.totalTime / BASE_LAP = number of laps completed (fractional)
    // + trackAnimTime = the animated offset that keeps everything moving
