...
function blankState(teamId){
  return {
    ...
    aduoReviewsDone: [],
    aduoHistory: [],
    wetWeekends: [],   // indices of tracks tagged as wet weekends this season
  };
}
...
function rollWetWeekends(){
  // Choose 3 European-leg rounds to be guaranteed wet weekends
...
function showSprintStrategyModal(){
  ...
  const bodyEl = document.getElementById('sprintStratBody');
  openModal(`
...
// ============ WEATHER CURVES ============
// Weekend weather pattern chosen at race start. Returns:
//   { curve, pattern, radarAt } where curve[lap] = wetness 0..1
//   pattern: 'dry' | 'dry_threat' | 'wet_drying' | 'dry_wet' | 'full_wet'
//   radarAt: array of lap indices where a "rain in 3 laps" warning should fire
function buildWeather(track, laps, isWetWeekend, isSprint){
  ...
  if(isSprint){
    // Sprints: no guaranteed weather, but wet weekends still apply
    if(isWetWeekend){
      const r = Math.random();
      if(r < 0.5){
        pattern = 'wet_drying';
  ...
  if(isWetWeekend){
    // Tagged wet weekend — one of three patterns, mostly flip-y
    const r = Math.random();
    if(r < 0.45){
      // wet start, rapid dry — crossover in first 30% of race
      pattern = 'wet_drying';
      const dryAt = Math.max(6, Math.floor(laps * randf(0.2, 0.4)));
      for(let l=0;l<laps;l++) curve.push(clamp(1 - l/dryAt, 0, 1));
    } else if(r < 0.80){
      // dry start, deluge arrives — 3-lap transition
      pattern = 'dry_wet';
  ...
  // Non-tagged weekend — use baseline behaviour
  ...
  // Dry baseline — small chance of rain on non-European tracks
...
function buildRaceState(track, gridOverride, opts){
  ...
  return {
    ...
    redFlagCount: 0,        // cap 1 per race
    incidentCount: 0,       // cap 3 SC/VSC events per race
    scheduledVscLap: 0,     // 0 = none scheduled
    scheduledVscFired: false,
...
function initRaceUI(track){
  ...
  // Schedule a VSC for non-sprint races
  if(!RACE.isSprint){
...
function simulateLap(silent){
  ...
  const wetTrend = wet - prevWet;   // positive = getting wetter
  ...
  // Radar warning — one-shot per race
  ...
  // Scheduled VSC floor — skip if under flag already, skip after late-race threshold,
  // skip if too many incidents already, skip in sprint
  ...
  // Battery + overtake + recharge resolution
  active.forEach((d)=>{
    ...
    // Recharge resolves at end of penalty lap
  ...
  // Aquaplaning red flag (only if no red flag yet, capped)
...
function computeLapTime(d, track, wet, wetTrend){
  ...
  // Tyre age loss with cliff at 25% life
  ...
  // Compounding penalty if wrong tyre AND trending wrong way
...
function checkIncidents(active, wet){
  active.forEach(d=>{
    ...
    // Crash chance scales with wetness aggressively
    ...
    if(roll < failChance){
      ...
      if(Math.random() < 0.72){
        ...
        // Wet crashes always produce a safety car, never just yellow
...
function retireDriver(d, reason, wet){
  ...
  if(reason==='crash'){
    const r = Math.random();
    // Red flag roll scaled by wetness
    let redChance = 0.15;
...
function maybeAIPitStop(d, track, wet, scRush){
  ...
  // AI crossover floor — pit within 2 laps of the crossover point
...
function renderTower(){
  ...
  RACE.drivers.forEach(d=>{
    ...
    // 2-compound warning — show after lap 5 in dry races
...
function showRaceResults(){
  ...
  RACE.drivers.forEach(d=>{
    ...
    if(isWetRace) return;   // wet race — no compound rule
    const dryUsed = new Set();
    ...
    if(dryUsed.size < 2){
      ...
      pushMsg('rc', 'RC:', `${d.abbr} DISQUALIFIED — only used ${usedList} (2 dry compounds required).`);
    }
  });
